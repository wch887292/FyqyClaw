...
4. ✅ **自动化测试** — 生成测试策略，并在桌面端经加固沙箱真实运行测试命令
5. 🔄 **错误修复** — 发现错误后自动尝试修复
...
| **API Key 本地加密** | 浏览器指纹 + 会话盐值混淆，禁止明文存储（当前为可逆混淆保护，后续版本将升级为系统密钥链 safeStorage 真加密） |
...
| 版本 | 状态 | 内容 |
|------|------|------|
| **V1.0.0** | ✅ 已发布 (2026-08-15) | 正式版发布，全流程 AI 辅助开发闭环 |
| **V1.1.0** | ✅ 已发布 (2026-08-16) | 可信化落地：沙箱生效、加密修复、Anthropic 真适配、Agent 真实写盘、配置页接真、MCP 真调用、插件系统、上下文真读、审查真代码、清理死依赖 |
| **V1.1.1** | ✅ 已发布 (2026-08-21) | 终极优化：主进程 fs 越界校验、Agent 写盘路径约束、沙箱策略防绕过 + 隐私禁网、API Key 加密真实化、技能/测试能力诚实化、自定义模型适配器路由、配置页真实取数、审查去 mock、补齐安全边界测试（69 测试全绿） |
...
| [FyqyClaw](https://github.com/wch887292/FyqyClaw) | 全流程 AI 驱动开发工具（IDE + AI Agent）（本仓库） |
